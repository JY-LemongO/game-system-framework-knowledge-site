...
JavaScript 실행 참조는 후속 반응의 순서, wave 상한, 실패 종료 조건을 trace에 남긴다. 현재 C# `ReactionQueue`는 trace port를 구현하지 않고 실패 예외와 dispatch 개수(<code>int</code>)만 제공하므로, 같은 관찰성이 필요한 제품 adapter는 별도 trace 연결을 추가해야 한다. 두 참조 모두 실패한 미dispatch 항목을 자동 재시도하지 않으므로 재시도가 필요한 제품은 별도 dead-letter/retry 정책을 명시해야 한다. 각 제품은 게임 규칙에 맞는 priority namespace와 budget 정책을 별도로 승인해야 한다.
